...
# Build the example
go build -o middleware_example main.go
...
ACTUAL_PUBLIC=$(curl -s http://localhost:3002/public)
...
ACTUAL_UNAUTH=$(curl -s http://localhost:3002/private/data)
...
ACTUAL_PRIVATE=$(curl -s -H "Authorization: valid-token" http://localhost:3002/private/data)
